...
从 [GitHub Releases](https://github.com/newstarbar/ModCrafting/releases) 或 [Gitee 发布页](https://gitee.com/newstarbar/ModCrafting/releases) 下载（**国内用户推荐 Gitee**）：

| 安装包 | 大小 | 网络 | 适用场景 |
|--------|------|------|----------|
| `ModCrafting Setup x.x.x.exe` | ~1–1.5 GB | 首次复制离线包，之后可离线 | **推荐**。日常开发、网络不稳定 |
| `ModCrafting x.x.x Portable.exe` | ~80–150 MB | **首次必须联网**下载工具链（约 1GB） | U 盘、临时机器、可接受首启下载 |
...
2. 打 tag 并推送：`git tag v1.0.x && git push origin v1.0.x`
