...
<img src="build/icon.png" alt="ModCrafting" width="128" height="128" />
...
从 [Releases](https://github.com/newstarbar/ModCrafting/releases) 下载：

| 安装包 | 适用场景 |
|--------|----------|
| `ModCrafting Setup x.x.x.exe` | 推荐。NSIS 向导安装，支持覆盖升级、自选目录 |
| `ModCrafting x.x.x Portable.exe` | 便携版，解压即用，适合 U 盘或免安装场景 |

安装后首次启动会显示**工具链初始化遮罩**（JDK → Gradle → 依赖），完成后即可使用。
...
| `npm run build:win:setup` | 构建 NSIS 安装程序 |
