...
const EnvImportDialog: React.FC<Props> = ({ onSuccess, onClose }) => {
  ...
  const handleImport = useCallback(async (zipPath: string) => {
  ...
  const phaseLabel = progress?.phase === 'preparing' ? '准备中'
    : progress?.phase === 'extracting' ? '解压中'
    : progress?.phase === 'verifying' ? '验证中'
      ...
      <div className="toolchain-init-card toolchain-init-card--download" style={{ maxWidth: 480 }}>
        ...
        {importing && progress && (
          <div style={{ padding: '8px 0' }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 12, lineHeight: 1.5 }}>
              正在解压环境包（约 1-2 GB），请耐心等待。解压完成后会自动验证环境完整性。
            </p>
          </div>
        )}
        ...
        {success && (
          <div style={{ padding: '16px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--success)', fontWeight: 600, marginBottom: 4 }}>
              环境导入完成
            </div>
