...
export function buildUserSymptomBlock(symptom: string | null | undefined): string {
  ...
  return (
    ...
    `硬约束：trigger_build runClient 出现 MC_PHASE:menu 仅表示游戏启动成功，不代表该症状已修复。` +
    `menu 后必须调用 mc_ensure_test_world 进入世界，再调用 mc_inspect 或 mc_screenshot（必要时 mc_inventory / mc_world / mc_command）做客观校验。` +
