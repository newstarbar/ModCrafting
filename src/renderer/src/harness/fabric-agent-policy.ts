...
const COMMON_GUARDRAILS = [
  ...
  'runClient 出现 MC_PHASE:menu 后，必须调用 mc_ensure_test_world 进入游戏世界（不能停在主菜单）。功能在游戏内的（HUD/方块/物品/实体/命令/事件）必须：① mc_ensure_test_world 进入世界 ② mc_ensure_cheats 确保作弊权限 ③ mc_test_scenario(feature_type=...) 获取测试步骤模板 ④ 按步骤执行测试场景（用 mc_command 生成生物/给予物品/切换模式、用 mc_input 移动玩家/触发交互） ⑤ mc_screenshot/mc_inspect/mc_world/mc_observe_entity 客观验证功能效果。禁止仅凭 MC_PHASE:menu 宣称功能完成。',
