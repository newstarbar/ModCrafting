...
const COMMON_GUARDRAILS = [
  ...
  'runClient 出现 MC_PHASE:menu 后，必须调用 mc_ensure_test_world 进入游戏世界（不能停在主菜单）。功能在游戏内的（HUD/方块/物品/实体/命令/事件）必须：① mc_ensure_test_world 进入世界 ② mc_ensure_cheats 确保作弊权限 ③ mc_test_scenario(feature_type=...) 获取测试步骤模板 ④ 按步骤执行测试场景（用 mc_command 生成生物/给予物品/切换模式、用 mc_input 移动玩家/触发交互） ⑤ mc_screenshot/mc_inspect/mc_world/mc_observe_entity 客观验证功能效果。禁止仅凭 MC_PHASE:menu 宣称功能完成。禁止仅凭"已进入世界"宣称功能完成。禁止跳过 mc_test_scenario 直接 complete_step。',
  'mc_test_scenario 的 feature_type 取值：new_item（新物品）/ new_block（新方块）/ new_recipe（新合成配方）/ entity_behavior（实体行为修改，如苦力怕爆炸改樱花、末影人移动方式修改）/ player_interaction（玩家交互功能，如闪电剑）/ hud_gui（HUD或界面）。获取步骤后按顺序执行，每步用客观证据验证。实体行为修改类功能必须用 mc_observe_entity 对比状态变化（如爆炸倒计时、AI 目标、移动速度），禁止仅凭截图宣称完成。',
  'mc_ensure_test_world 失败后，必须重试或用 mc_inspect 检视当前界面后用 mc_input click_widget 手动操作进入/创建世界，禁止跳过世界进入步骤直接宣称测试完成。若自动创建世界失败（按钮文案不匹配等），必须 mc_inspect 获取 widgets 列表，用 index 点击对应按钮。',
