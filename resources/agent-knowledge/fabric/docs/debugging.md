# debugging
> 来源: https://docs.fabricmc.net/zh_cn/develop/debugging

# 调试模组 26.1.2 ​

了解有关日志记录、断点和其他有用的调试功能的所有信息。
...
ExampleMod.LOGGER.error("Critical exceptions, bugs...");
```
...
- `Time` - 打印日志信息的时间
- `Thread` - 打印信息的线程。 你通常会看到一个 服务器线程 和一个 渲染线程，这告诉你代码是在哪一端执行的
...
`debug` 级别默认是隐藏的，要在开发期间显示它，请参阅调试日志记录。
...
日志记录器会在行首打印你的模组 ID。 你可以搜索 ( ⌘/CTRL + F ) 来高亮显示。
...
我们再次以自定义物品为例。 该物品的 `CUSTOM_NAME` `DataComponentType` 应该会根据其在任何石头方块上使用而改变。 然而，在这个例子中，该物品似乎总是会触发“成功”手势动画，而圆石似乎并没有改变其名称。
...
操作 说明 步过（Step Over） 进入下一行执行，基本上是沿着逻辑移动实例 步入（Step In） 进入方法内部，显示正在发生的事情。 如果一行中有多个方法，你可以通过点击来选择要进入的方法。 这对于 Lambda 表达式也是必需的。 运行到光标处 逐步执行逻辑，直到到达代码中的光标。 这对于跳过大段代码非常有用。 显示执行点 将编码窗口的视图移动到调试器当前所在的位置。 即使你当前正在处理其他文件和选项卡，此操作也有效。

INFO

“步过” F8 和“步入” F7 操作是最常见的，因此请尝试熟悉这些快捷键！

如果你完成了当前检查，你可以按绿色的 `恢复程序` 按钮（ F9 ）。 这将解冻 Minecraft 实例，并且可以进行进一步的测试，直到遇到另一个断点。 但现在我们先继续查看 `调试` 窗口。
...
此过程也称为“热交换”，要求 Minecraft 实例以 `调试` 模式而不是 `运行` 模式启动（参见上文）。

这样，你就不需要再次重新启动 Minecraft 实例。 它还可以更快地测试屏幕元素对齐和其他功能平衡。 如果“热交换”成功，IntelliJ 将通知你。

Mixin 是个例外。 你可以设置运行配置，允许它们在运行时更改。 更多信息，请参阅[热交换 Mixin](./getting-started/launching-the-game#hotswapping-mixins)。

其他更改可以在游戏中重新加载。

- `assets/` 文件夹的更改 -> 按 F3+T
