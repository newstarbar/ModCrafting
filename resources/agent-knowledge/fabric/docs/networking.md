# networking
> 来源: https://docs.fabricmc.net/zh_cn/develop/networking

# 网络通信 26.1.2 ​

使用 Fabric API 进行网络通信的一般指南。
...
请留意数据包是如何充当服务器和客户端之间的桥梁的；这是因为你在游戏中所做的几乎一切都以某种方式涉及网络。 例如，当你发送一条聊天消息时，包含内容的数据包会发送到服务器。 然后，服务器会向所有其他客户端发送包含你消息的另一个数据包。
...
让我们从创建 Lightning Tater 物品开始。 你可以重写 `use` 以在使用该物品时触发操作。 本例中我们向服务器世界的玩家发送数据包。
...
然后我们在**通用初始化程序**中注册我们的有效载荷。 然而，这次通过使用 `PayloadTypeRegistry.clientboundPlay().register` 作为 *客户端到服务器* 有效载荷。
...
此外，目标实体必须是生物实体，并且我们将目标实体的范围限制在距离玩家 5 的​​范围内。 如果满足这些条件，我们将应用该效果：
