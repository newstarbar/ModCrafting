# first fluid
> 来源: https://docs.fabricmc.net/zh_cn/develop/fluids/first-fluid

# 创建你的第一个流体 26.1.2 ​

学习如何在 Minecraft 中创建你的第一个自定义流体方块。
...
不要忘记，为了正确渲染，物品需要名称为 `acid_bucket` 的翻译、[纹理](./../items/first-item#adding-a-texture)、[模型](./../items/first-item#adding-a-model)以及[客户端物品](./../items/first-item#creating-the-client-item)。 下面提供了一个示例纹理。 [下载 纹理](/assets/develop/fluids/acid_bucket_small.png)

另外，建议将你的模组中的桶添加到 `ConventionalItemTags.BUCKET` 物品标签中，这样其他模组就能正确处理它。你可以手动添加，也可以通过[数据生成](./../data-generation/tags)添加。
