...
尽管 Minecraft 的许多渲染都通过 `GuiGraphics` 中的各种方法抽象出来，且您很可能并不需要接触这里提到的任何内容，但是了解渲染的基础实现依然很重要。

## 镶嵌器 `Tesselator`{#the-tessellator}

镶嵌器 `Tesselator` 是 Minecraft 中用于渲染东西的主类。 它是一个单例，这意味着游戏中只有它的一个实例。 您可以通过 `Tesselator.getInstance()` 获取这个实例。
...
| 绘制模式                        | 描述                                               |
| --------------------------- | ------------------------------------------------ |
| `Mode.LINES`            | 每个元素由 2 个顶点构成，代表一条直线。                            |
| `Mode.LINE_STRIP`       | 第一个元素需要 2 个顶点。 附加的元素只需要 1 个新的顶点就能绘制，创建出一条连续的线。   |
| `Mode.DEBUG_LINES`      | 与 `Mode.LINES` 类似，但线在屏幕上总是保持 1 像素宽。          |
| `Mode.DEBUG_LINE_STRIP` | 与 `Mode.LINE_STRIP` 相同，但线总是保持 1 像素宽。         |
| `Mode.TRIANGLES`        | 每个元素都由 3 个顶点组成，形成一个三角形。                          |
| `Mode.TRIANGLE_STRIP`   | 前 3 个顶点形成第一个三角形。 每个新增的顶点与前两个顶点形成一个新的三角形。         |
| `Mode.TRIANGLE_FAN`     | 前 3 个顶点形成第一个三角形。 每个新增的顶点与第一个顶点和最后的一个顶点形成一个新的三角形。 |
| `Mode.QUADS`            | 每个元素由 4 个顶点构成，形成一个四边形。                           |

### 向 `BufferBuilder` 写入{#writing-to-the-bufferbuilder}

`BufferBuilder` 初始化完成后，您就可以向它写入数据。

`BufferBuilder` 允许我们一个顶点一个顶点地构造我们的缓冲。 我们调用 `buffer.addVertex(matrix, float, float, float)` 方法来添加一个顶点。 参数 `matrix` 是变换矩阵，具体细节我们将在后文中讨论。 3 个 `float` 参数代表顶点坐标的 (x, y, z)。
...
通常是通过 `PoseStack` 类获取的， `PoseStack` 实例可以从 `GuiGraphics` 对象获取：
...
用实际案例来解释如何向 `BufferBuilder` 写入会更轻松一些。 不妨让我们尝试用绘制模式 `Mode.TRIANGLE_STRIP` 和顶点格式 `POSITION_COLOR` 来渲染一些东西。
...
从上面的案例出发，我们可以用 `PoseStack` 和 `tickDelta`（从上一帧到现在经过的时间）让菱形放大和缩小。 我们稍后会在 [渲染 HUD](./hud#render-tick-counter) 页面中澄清这一点。
...
四元数是三维空间中的旋转的一种表示方法， 用于通过 `mulPose(Quaternion, x, y, z)` 方法旋转 `PoseStack` 栈顶的矩阵。

您几乎不太可能需要直接使用 `Quaternion` 类，因为 Minecraft 在工具类 `RotationAxis` 中提供了许多预先定义的 `Quaternion` 实例。

不妨让我们尝试绕 z 轴旋转我们的菱形。 我们可以通过 `PoseStack` 和 `mulPose(Quaternion, x, y, z)` 方法来实现。
