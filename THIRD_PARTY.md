...
> - 安装包面向最终用户的补充说明见 [`build/license.txt`](build/license.txt)。
...
Windows 安装包由 [electron-builder](https://www.electron.build/) 调用 **NSIS** 生成。NSIS 及相关插件许可见其官方发布包。自定义安装逻辑见 [`build/installer.nsh`](build/installer.nsh)。
